...
const Expression = new Driver('Expression').match('LParen').consumeRight(ArgumentList, 99).until(type('RParen'))
const FunctionCall = new Driver('FunctionCall').match('LParen').consumeLeft(Identifier).consumeRight(ArgumentList, 99).until(type('RParen'));
const Comparison = new Driver('Comparison').match(type('Comparison')).consumeLeft(FunctionCall).consumeRight(FunctionCall, 1).end('RParen');
// const Greater = new Driver('Greater').match(type('LessThen')).consumeLeft(FunctionCall).consumeRight(FunctionCall, 1).end('RParen');
const And = new Driver('AND').match(value('AND')).consumeLeft(Expression).consumeRight(FunctionCall, 99).until(value('AND')).repeat().end('RParen');
